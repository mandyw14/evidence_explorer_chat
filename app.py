...
def generate_lay_titles(
    results_df: pd.DataFrame,
    client: OpenAI,
    max_articles: int = 10,
) -> dict[str, str]:
    """Generate one concise plain-language title for each abstract preview."""
    if results_df.empty:
        return {}
        ...
        title_records.append(
            {
                ...
                "abstract": str(row.get("Abstract", "")).strip()[:5000],
            }
        )

    system_prompt = """
You create accurate plain-language titles for scientific articles.

For each supplied PubMed record:
- Write one clear lay title that a general reader can understand.
- Base it only on the scientific title and abstract provided.
- Do not exaggerate findings or imply causation unless the abstract supports it.
- Do not give medical advice.
- Keep each lay title to 14 words or fewer.
- Return valid JSON only, using this structure:
  {"titles": [{"pmid": "123", "lay_title": "Plain-language title"}]}
"""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": json.dumps({"records": title_records}, ensure_ascii=False),
            },
        ],
    )
    ...
    for item in parsed.get("titles", []):
        ...
        if pmid and lay_title:
            lay_titles[pmid] = lay_title
...
if search:
    ...
    if "Lay_Title" not in df.columns:
        df["Lay_Title"] = ""

    if client is not None and not df.empty:
        with st.spinner("Creating plain-language article titles…"):
            try:
                lay_titles = generate_lay_titles(df, client)
                df["Lay_Title"] = df["PMID"].astype(str).map(lay_titles).fillna("")
            except Exception:
                # Keep the PubMed search usable if lay-title generation fails.
                df["Lay_Title"] = ""
...
if not results_df.empty:
    ...
    for _, row in preview_df.iterrows():
        ...
        with st.expander(expander_title):
            if lay_title:
                st.caption("Plain-language title generated from the PubMed title and abstract.")
